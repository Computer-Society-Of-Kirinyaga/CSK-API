...
export const resetUserLoginCredentials = async (req, res) => {
    ...
    if (user) {
        // send an email to the user email with the password
        const response = await sendEmail(email, 'Password Reset', `Your password is  ${user.password}, please keep it safe and use it to login.`);
