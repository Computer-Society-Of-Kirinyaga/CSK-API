...
router.post("/", upload.single("filename"), async (req, res) => {
    try {
        ...
        const storageRef = ref(
            storage,
            `blogFiles/${req.file.originalname}_${randomString}_${dateTime}`
        );
        ...
        return res.status(200).send({
            message: "blogFile uploaded to Firebase Storage",
            viewURL: viewURL,
            name: file.originalname,
        });
    } catch (error) {
        return res.status(500).send("Error uploading blogFile:", error);
    }
});
