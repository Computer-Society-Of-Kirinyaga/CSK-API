    ...
    const handler = async (req, res) => {
        ...
        if (!name && !description && !imagesURL && !uploadedBy
            ...
            handleValidationError(
                { details: [{ message: "At least gallery one property must be provided" }] },
                res
            );
            return;
        }
...
export const getGalleries = async (req, res) => {
    const handler = async (req, res) => {
        const galleriesResponse = await GalleryModel.find();
        galleriesResponse.length > 0
            ? res.status(200).json(galleriesResponse)
...
export const updateGallery = async (req, res) => {
    const handler = async (req, res) => {
        ...
        if (!name && !description && !imagesURL && !uploadedBy) {
            handleValidationError(
                { details: [{ message: "At least one gallery property must be provided" }] },
                res
            );
            return;
        }
