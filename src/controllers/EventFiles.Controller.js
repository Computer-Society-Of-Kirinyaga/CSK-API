...
router.post("/", upload.single("filename"), async (req, res) => {
    try {
        ...
        const file = req.file;
        const storageRef = ref(storage, `eventFiles/${file.originalname}_${randomString}_${dateTime}`);
        const metadata = { contentType: file.mimetype };        // Create file metadata including the content type        
        const snapshot = await uploadBytesResumable(storageRef, file.buffer, metadata); // Upload the file in the bucket storage
    ...
    try {
        ...
        req.files && req.files.forEach((file) => {
            ...
            if (file.originalname.indexOf('.') !== -1) {
                ...
                newFileName = file.originalname.replace(`.${fileExtension}`, `_${randomString}_${dateTime}.${fileExtension}`);
            } else {
                newFileName = `${file.originalname}_${randomString}_${dateTime}`;
            }
            const storageRef = ref(storage, `eventFiles/${newFileName}`);
            const metadata = { contentType: file.mimetype }; // Create file metadata including the content type           
            const uploadPromise = uploadBytesResumable(storageRef, file.buffer, metadata)  // Upload the file in the bucket storage
                .then(async (snapshot) => {
                    const viewURL = await getDownloadURL(snapshot.ref); // Grab the public URL
...
router.delete("/:name", async (req, res) => {
    if (req.params.name == '[""]' || !req.params.name || req.params.name == null || req.params.name == undefined) return res.status(400).send('No file names provided url params');
    ...
    deleteObject(storageRef).then(() => {
        ...
        } else return res.status(500).send(`Error deleting "${name}" from Firebase Storage`);
    });
...
router.delete("/many/:filesArray", async (req, res) => {
    if (req.params.filesArray == '[""]' || !req.params.filesArray || req.params.filesArray == null || req.params.filesArray == undefined) return res.status(400).send('No file names provided in the url params');
    ...
    names.forEach((name) => {
        ...
        const deletePromise = deleteObject(storageRef)
            .then(() => {
                ...
                if (error.code === 'storage/object-not-found') {
                    return name;
                } else return res.status(500).send('Error deleting file from Firebase Storage' + error);
            });
