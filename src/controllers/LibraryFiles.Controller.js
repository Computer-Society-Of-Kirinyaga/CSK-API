...
router.post("/", upload.single("filename"), async (req, res) => {
    try {
        ...
        const storageRef = ref(storage, `libraryFiles/${req.file.originalname}_${randomString}_${dateTime}`);
...
router.post("/many", upload.array("filename", 5), async (req, res) => {
    ...
    try {
        ...
        req.files.forEach((file) => { // Loop through each uploaded file and upload it to Firebase Storage
            const randomString = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
            const storageRef = ref(storage, `libraryFiles/${file.originalname}_${randomString}_${dateTime}`);
...
router.delete("/many/:filesArray", async (req, res) => {
    if (req.params.filesArray == '[""]' || !req.params.filesArray || req.params.filesArray == null || req.params.filesArray == undefined) return res.status(400).send('No file names provided url params');
