...
export const getEvents = async (req, res) => {
    const handler = async (req, res) => {
        ...
        eventsResponse.length > 0 ? res.status(200).json(eventsResponse) : res.status(404).json({ message: 'No Events found' });
    };
    tryCatchWrapper(handler, req, res);
};
...
export const getPastEvents = async (req, res) => {
    const handler = async (req, res) => {
        ...
        pastEvents.length < 0 ? res.status(200).json(pastEvents) : handlePastEventNotFound(res);
    };
    tryCatchWrapper(handler, req, res);
};
...
export const getFutureEvents = async (req, res) => {
    const handler = async (req, res) => {
        ...
        futureEvents.length < 0 ? res.status(200).json(futureEvents) : handleFutureEventNotFound(res);
    };
    tryCatchWrapper(handler, req, res);
};
